...
α = .6
γ_b = .01
γ_c = (1-ε) * ( ( 3 * u_mf / ε_mf ) / (u_b - u_mf / ε_mf) + α )
γ_e = (1-ε) * (1-f_b)/f_b
...
Ga = g * d_p^3 * (g * (ρ_cat - ρ_gas) * ρ_gas) / μ^2
...
# write results            
YAML.write_file((pth_out * "fluidisation_parameters.yml"), fluidization)
